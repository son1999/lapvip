var app = new Vue({
    // options content
    el: '#gallery-product-vue',
    data: searchDataProduct,
    mounted(){
    ...
    methods:{
        ...
        loadImages:function(e){
            if(typeof this.object_id != 'undefined' && this.object_id > 0) {
                ...
                this.loading = true;
                shop.ajax_popup('story_wt/loadproduct', 'POST', data,
                    function (json) {
                        app.loading = false;
                        if (json.error == 0) {
        ...
        imageDel:function(item)     {
        	if(confirm('Bạn muốn xóa ảnh?')) {
                this.loading = true;
                shop.ajax_popup('story_wt/remove_img_product', 'POST', {id: item.id, object_id:this.object_id,type:this.type},
                    function (json) {
                        app.loading = false;
                        let msg = json.msg;
                        if (json.error == 0) {
