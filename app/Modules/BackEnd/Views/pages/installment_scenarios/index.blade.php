            ...
            <div class="card-body">
                <table class="table table-bordered table-striped table-responsive">
                    ...
                    <tr>
                        ...
                        <td>{{$item->title}}</td>
                        <td>{{ $item->month }} tháng</td>
                        <td align="center">{{ \Lib::dateFormat($item->created, 'd/m/Y') }}</td>
                        <td align="center">
                            @if(\Lib::can($permission, 'edit'))
                                @if($item->status == 2)
                                    <a href="javascript:void(0)" class="text-primary" onclick="shop.admin.updateStatus({{ $item->id }},false,'installment_scenarios')" title="Đang hiển thị, Click để ẩn"><i class="fe-check-circle"></i></a>
                                @else
                                    <a href="javascript:void(0)" class="text-secondary" onclick="shop.admin.updateStatus({{ $item->id }}, true,'installment_scenarios')" title="Đang ẩn, Click để hiển thị"><i class="fe-check-circle"></i></a>
                                @endif
