        ...
        <div class="card card-accent-info">
            ...
            <div class="card-body">
                <table class="table table-bordered table-striped table-responsive">
                    ...
                    <tbody>
                    @php($groupArr = $group->keyBy('id')->toArray())
                    @foreach ($data as $customer)
                    <tr>
                        <td align="center">{{ $customer->id }}</td>
                        <td align="center">
                            @if(!empty($customer->avatar))
                                <img data-src="{{ $customer->getImageUrl('small') }}" class="float-left lazyload" width="80">
                            @else
                                ---
                            @endif
                        </td>
                        ...
                        <td>
                            @foreach($customer->groups as $g)
                                @if(!empty($groupArr[$g->id]))
                                    <div>
                                        + {{ $groupArr[$g->id]['title'] }}
                                    </div>
                                @endif
                        ...
                        {{--</td>--}}
                        @if(\Lib::can($permission, 'edit'))
                            <td align="center"><a href="{{ route('admin.'.$key.'.edit', $customer->id) }}" class="text-primary"><i class="fe-edit"></i></a></td>
                        @endif
                        @if(\Lib::can($permission, 'delete'))
